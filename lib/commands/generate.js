...
async function generateImages(activeConfig) {
    ...
    for (const combination of allCombinations) {
        ...
        // Check if preview is set in activeConfig.launch
        if (activeConfig.launch.preview) {
            console.log('Preview mode: The following parameters would be used:');
            console.log('URL:', `${activeConfig.api.baseURL}/${config.apiService.baseGenURLPath}/txt2img`);
            console.log('Parameters:', JSON.stringify({ ...combination.config, prompt: prompt }, null, 2));
            continue;
        }
