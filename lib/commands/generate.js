...
function getGenerations(activeConfig) {
    ...
    prompt_combinations.forEach(prompt => {
        param_combinations.forEach(params => {
            ...
            //relevant params: collect param values only where params are changing under script control. 
            relevant_params = Object.keys(dynamic_config).reduce((acc, key) => {
                if (key in params) {
                    acc[key] = params[key];
                }
                return acc;
            ...
            generations.list.push({
                ...
                params,
                metadata: { 
                    generation_id: `${generations_id}_${generation_number}`,
...
async function generateImages(activeConfig) {
    ...
    for (const generation of generations.list) {
        ...
        const generation_id = generation.metadata.generation_id;
        const prompt = generation.prompt;
        const params = generation.params;
        const relevant_params = generation.metadata.relevant_params;
        const filename = utils.imageFilename(prompt, relevant_params, generation_id);
