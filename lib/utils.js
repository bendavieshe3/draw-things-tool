...
function imageFilename(prompt, params, project_title='default') {
    const paramString =  Object.entries(params)
        .map(([key, value]) => `${key}_${value}`)
        .join('_');
    const promptString = sanitizeFilename(capitalCase(prompt));
    return `${project_title}_${promptString}_${paramString}.png`
}
